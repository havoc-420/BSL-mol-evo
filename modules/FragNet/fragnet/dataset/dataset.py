...
# Set OpenMP thread limit to prevent resource issues
...
# Get number of CPUs, but limit to prevent resource issues
max_cpus = min(2, os.cpu_count() or 2)  # Default to 2 if os.cpu_count() returns None
...
def get_pt_dataset(df, data_type, maxiters=200, frag_type="brics"):
    ...
    st = time.time()
    # Limit the number of threads to prevent resource issues
    train_dataset = Pool(max_cpus).imap(create_data.create_data_point, x)
    print(time.time() - st)
...
class FinetuneData:
    ...
    def get_ft_dataset(self, df):
        ...
        st = time.time()
        # Limit the number of threads to prevent resource issues
        train_dataset = Pool(max_cpus).imap(self.create_data.create_data_point, x)
        print(time.time() - st)
