...
def train_nnconv_model(data_file: str, max_pairs: int = None, epochs: int = 100, seed: int = 42, 
    ...
    try:
        ...
        # 准备属性变化目标
        if normalize:
            target_features = prepare_property_change_targets_selected(data_file, property_stats, max_pairs, selected_properties)
        else:
            target_features = prepare_property_change_targets_no_standardization(data_file, max_pairs, selected_properties)
            # 创建空的属性统计信息，表示未进行标准化
            property_stats = {}
        ...
        with torch.no_grad():
            ...
            # 保存模型
            model_filename = "molecule_evolution_nnconv_predictor.pth"
