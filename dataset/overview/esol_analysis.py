...
def analyze_esol_dataset():
    ...
    try:
        ...
        # 加载ESOL数据集
        dataset = MoleculeNet(root='/Users/havoc420/Documents/Projects/python/mol-evo/data/MoleculeNet', 
                             name='ESOL')
        ...
        for i in range(len(dataset)):
            ...
            if mol is not None:
                ...
                # 如果无法解析，使用默认值
                num_heavy_atoms = data.num_nodes  # 备用方法
                heavy_atom_counts.append(num_heavy_atoms)
        ...
        print(f"导入错误: {e}")
        print("请确保已安装必要的库:")
        print("pip install torch torch-geometric matplotlib rdkit")
        print("\n如果尚未安装ESOL数据集，首次运行时会自动下载")
        return None
...
def create_visualization(distribution, total_molecules):
    ...
    try:
        ...
        fig.suptitle('ESOL Dataset Statistics (Heavy Atoms)', fontsize=20, fontweight='bold')
        ...
        # 添加统计信息文本
        stats_text = f"""
        ESOL Dataset Statistics (Heavy Atoms)
        ...
        plt.tight_layout()
        filename = 'esol_dataset_visualization.png'
...
def main():
    # 分析数据集
    distribution = analyze_esol_dataset()
