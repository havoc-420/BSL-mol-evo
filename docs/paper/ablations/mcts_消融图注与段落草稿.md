...
### 4.2 附录表格 caption（含 `random_topb` 与 official 失败说明）

**中文**：

表 Y：MCTS 侧扩展消融结果。除主文中的 `wo_prior` 与 `wo_leaf_value` 外，我们还报告随机 `top-b` expansion ranking 的结果。该变体在当前证据下主要表现为搜索效率下降，而其对最终优化质量的影响仍具有一定任务依赖性。另在 `official` 预算的搜索层实验中，`homo_down / wo_leaf_value` 出现了 `3 / 3` seeds 的复现失败，可作为稳定性风险的补充证据。

**English**:

Table Y: Extended MCTS ablation results. In addition to `wo_prior` and `wo_leaf_value`, we also report the variant with random top-b expansion ranking. Under the current evidence, this variant mainly reduces search efficiency, while its impact on final optimization quality remains task-dependent. In the official-budget search-only run, `homo_down / wo_leaf_value` further shows reproducible failure on all three seeds, providing extra evidence of instability after removing the leaf-value signal.
...
- 本轮 `official` 进一步显示，`homo_down / wo_leaf_value` 在 `3 / 3` seeds 上发生复现失败。
...
- In the official-budget search-only run, `homo_down / wo_leaf_value` further fails reproducibly on all three seeds.
- The degradation caused by removing the prior is directionally consistent but much smaller than that of the leaf-value signal.
...
## 6. 若后续要形成最终定稿，还需要更新什么

如果后续补齐 `official` 的失败 run 并完成真值评估，本文件最需要更新的地方有三类：

1. 把结果段中的数值改成多 seed 真值均值；
2. 视最终结果决定 `wo_prior` 是继续保留在主文，还是退到附录；
3. 决定 `official` 的失败信息是保留在主文讨论段，还是转移到附录说明。

其余大部分叙述逻辑大概率不需要推翻。

## 7. 一句话总结

当前最值得提前写下来的不是更多原始数字，而是：**围绕 `wo_leaf_value` 这个最强负面对照，把正文结果段、讨论段、caption 和“稳定性风险”说明一起写成型。**
