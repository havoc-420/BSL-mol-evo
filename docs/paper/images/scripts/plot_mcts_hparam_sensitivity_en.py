#!/usr/bin/env python3
"""English-only variant of the MCTS hyperparameter sensitivity figure.

This mirrors `plot_mcts_hparam_sensitivity.py` but replaces all CJK labels with
English equivalents so the figure can be used in English manuscripts.

Outputs:
    - `mcts_hparam_sensitivity-v1-en.pdf`
    - `mcts_hparam_sensitivity-v1-en.png`
saved under the local paper image dir, and (if present) also synced to
`whu-thesis/pages/images`.
"""
...
C_SUCCESS = "#7f7f7f"
PARAM_LABELS_EN = {
    "num_simulations": "Search budget",
    "max_depth": "Maximum depth",
    "pruning_patience": "Pruning patience",
    "exploration_weight": "Exploration coefficient",
    "exploration_constant": "Exploration coefficient",
}

X_LABEL_FONT_KW = {"fontweight": "bold", "fontsize": 12}
...
def _plot_consistent_panel(ax, data: pd.DataFrame, x_col: str, xlabel: str) -> None:
    ...
    ax.set_xlabel(xlabel, **X_LABEL_FONT_KW)
...
def main() -> None:
    ...
    _plot_consistent_panel(
        ...
        budget,
        x_col="param_value",
        xlabel=f"(a) {PARAM_LABELS_EN['num_simulations']}",
    )
    _plot_consistent_panel(
        axes[1],
        explore,
        x_col="param_value",
        xlabel=f"(b) {PARAM_LABELS_EN['exploration_weight']}",
    )
    _plot_consistent_panel(
        axes[2],
        depth,
        x_col="param_value",
        xlabel=f"(c) {PARAM_LABELS_EN['max_depth']}",
    )

    saved_paths = []
    for output_dir in OUTPUT_DIRS:
        output_dir.mkdir(parents=True, exist_ok=True)
        out_pdf = output_dir / "mcts_hparam_sensitivity-v1-en.pdf"
        out_png = output_dir / "mcts_hparam_sensitivity-v1-en.png"
