...
class FragNetLayerA(nn.Module):
    ...
    def forward(
        ...
        )  # sum of attention weights attributable to the target atom
        ...
        summed_attn_weights_frags = scatter_add(
            attn_probs, source, dim=0
        )  # total attention attributed to each node in the frag graph, or each edge in the frag graph
...
class FragNetViz(FragNet):
    ...
    def __init__(
        ...
        self.layers[num_layer - 1] = FragNetLayerA(
            atom_in=emb_dim,
            atom_out=emb_dim,
            frag_in=emb_dim,
            frag_out=emb_dim,
    ...
    def forward(self, batch):
        ...
        x_atoms, x_frags, edge_features = self.layers[0](
            x_atoms,
            edge_index,
            edge_attr,
            frag_index,
        ...
        for layer in self.layers[1:-1]:
            x_atoms, x_frags, edge_features = layer(
                x_atoms,
                edge_index,
                edge_features,
                frag_index,
        ...
        x_atoms, x_frags, edge_features, attn_atom, attn_frag, attn_bond = self.layers[
            -1
        ](
            x_atoms,
            edge_index,
...
class FTHead1(nn.Sequential):
    ...
    def __init__(self, emb_dim=128, h1=128, drop_ratio=0.2, n_classes=1):
        super().__init__()
...
class FTHead4(nn.Module):
    ...
    def __init__(self, input_dim=128, h1=128, act="relu", n_classes=1, drop_ratio=0.2):
        super().__init__()
...
class FTHead2(nn.Sequential):
    def __init__(self, input_dim=128, h1=128, drop_ratio=0.2, n_classes=1):
        super().__init__()
...
class FragNetFineTuneTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.atom_transformer = TransformerConv(
            in_channels=emb_dim, out_channels=emb_dim, heads=transformer_heads
        )
        self.frag_transformer = TransformerConv(
            in_channels=emb_dim, out_channels=emb_dim, heads=transformer_heads
        )
...
class FragNetFineTuneMultiTask(FragNetFineTune):
    ...
    def __init__(self, n_classes=1, num_layer=4, drop_ratio=0.15, n_multi_task_heads=0):
        super().__init__(
            n_classes=n_classes, num_layer=num_layer, drop_ratio=drop_ratio
        )

        if n_multi_task_heads > 0:
            self.ms_heads = nn.ModuleList(
                [nn.Linear(128 * 2, n_classes) for i in range(n_multi_task_heads)]
            )
