...
# -*- coding: utf-8 -*-
"""
基于VisNet的分子进化预测器 v0.1 (Linear-Linear版本)
...
与v0版本相比，v0.1版本增加了特征差值计算模块，将目标分子特征减去起始分子特征作为边特征的一部分。
"""
...
@register_model(display_name="VisNet Linear-Linear 模型 v0.1", save_dir_name="visnet_linear_linear_v0_1")
class MoleculeEvolutionVisnetLinearPredictorV01(nn.Module):
    """
    基于VisNet的分子进化预测器 v0.1 (Linear-Linear版本)
    ...
    def __init__(self, node_feature_dim: int = 11, edge_feature_dim: int = 15,
        ...
        """
        super(MoleculeEvolutionVisnetLinearPredictorV01, self).__init__()
        ...
        # edge组件: 线性边特征编码器
        # 输入维度需要增加特征差值维度 (512)
        self.edge_encoder = LinearEdgeFeatureExtractor(edge_feature_dim + 512, hidden_dims[-1])
    ...
    def forward(self, from_data: Data, to_data: Data, edge_attr: torch.Tensor) -> torch.Tensor:
        ...
        # 计算特征差值: to_features - from_features
