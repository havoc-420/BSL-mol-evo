    ...
    def generate_expansion_tree(self, max_depth: int = 3, max_branching: int = 5, 
        ...
        for depth in range(max_depth):
            ...
            if all_candidates:
                ...
                # 计算前5%的候选数量，至少取1个，最多200个
                top_5_percent_count = max(1, min(200, int(len(all_candidates) * 0.05)))
                print(f"第 {depth} 层总候选数: {len(all_candidates)}，前5%候选数: {top_5_percent_count}")
                
                # 处理前5%的候选
                next_level = []
                for operation, new_smiles, property_change, parent_id in all_candidates[:top_5_percent_count]:
                    # 避免重复分子
                    if new_smiles not in seen_molecules:
