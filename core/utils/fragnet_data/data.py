...
def get_incr_atom_nodes(data_list):
    ...
    if len(data_list) == 1:
        ...
        incr_atom_nodes = torch.cat(
            [torch.zeros(data_list[0].edge_index.shape[1]), incr_atom_nodes], dim=0
        )
...
def get_incr_frag_nodes(data_list):
    ...
    if len(data_list) == 1:
        ...
        incr_atom_nodes = torch.cat(zeros)
        incr_atom_nodes = torch.cat(
            [torch.zeros(data_list[0].frag_index.shape[1]), incr_atom_nodes], dim=0
        )
    return incr_atom_nodes
...
def get_incr_atom_id_frag_id_nodes(data_list):
    ...
    if len(data_list) == 1:
        ...
        incr_atom_nodes = torch.cat(zeros)
        incr_atom_nodes = torch.cat(
            [torch.zeros(data_list[0].atom_id_frag_id.shape[0]), incr_atom_nodes], dim=0
        )
    return incr_atom_nodes
...
def get_incr_bond_nodes(data_list):
    ...
    if len(data_list) == 1:
        ...
        for i in range(1, len(data_list)):
            ...
            zeros.append(
                torch.zeros(data_list[i].edge_index_bonds.shape[1]) + max_bonds
            )

        incr_bond_nodes = torch.cat(zeros)
        incr_bond_nodes = torch.cat(
            [torch.zeros(data_list[0].edge_index_bonds.shape[1]), incr_bond_nodes],
            dim=0,
        )
...
def get_incr_fbond_nodes(data_list):
    ...
    if len(data_list) == 1:
        ...
        for i in range(1, len(data_list)):
            ...
            zeros.append(
                torch.zeros(data_list[i].edge_index_fbondg.shape[1]) + max_bonds
            )

        incr_bond_nodes = torch.cat(zeros)
        incr_bond_nodes = torch.cat(
            [torch.zeros(data_list[0].edge_index_fbondg.shape[1]), incr_bond_nodes],
            dim=0,
        )
    return incr_bond_nodes
...
def add_one_bond_frag_nodes_to_index(
    edge_index_bond_graph, bond_index_to_id, one_bond_frags
):
...
def get_edge_attr_bond_graph(edge_index, idx_bond_index, conf, one_bond_frag_ids=[]):
    ...
    for j in range(len(edge_index[0])):
        ...
        if (node1, node2) in one_bond_frag_ids:
            ...
            c = conf
            ang = Chem.rdMolTransforms.GetAngleRad(
                c, int(others[0]), int(common), int(others[1])
            )
            cos_ang = np.cos(ang)
...
def get_bond_angle_dhangle(conf, x_atoms, edge_index):
    ...
    bond_length_true = torch.sum(
        (bond_length_pair[:, 0, :] - bond_length_pair[:, 1, :]) ** 2, axis=1
    )
    ...
    for i in range(edge_index.shape[1]):
        rej_pos = (
            direction_unit[edge_index[0][i]]
            - torch.dot(direction_unit[edge_index[0][i]], unit_vector[i])
            * unit_vector[i]
        )
        rej_neg = (
            direction_unit[edge_index[1][i]]
            - torch.dot(direction_unit[edge_index[0][i]], unit_neg_vector[i])
            * unit_neg_vector[i]
        )
...
def get_fragbond(frag_idx, cnx_attr):
    ...
    node_feautures_fragbond_graph = torch.tensor(
        node_feautures_fragbond_graph, dtype=torch.float
    )
    ...
    for j in range(len(edge_index_fragbond_graph[0])):
        ...
        edge_feature = list(
            np.array(frag_bond_cnx_attr[tuple(node_1_bond)])
            + np.array(frag_bond_cnx_attr[tuple(node_2_bond)])
        )

        edge_attr_fragbond.append(edge_feature)

    edge_attr_fragbond = np.array(edge_attr_fragbond)
...
class CreateData:
    ...
    def create_data_point(self, args):
        ...
        x_atoms, edge_index, edge_attr = (
            self.feature_creator.get_atom_and_bond_features_atom_graph_one_hot(
                graph.mol, self.feature_creator.use_bond_chirality
            )
        )

        #### commmenting this section #### Ma4 28 2024
        # no edges
        if len(edge_index[0]) == 0 or (
            not (len(x_atoms) == max(edge_index[0]) + 1 == max(edge_index[1]) + 1)
        ):
            return None
        ...
        if self.create_bond_graph_data:
            ...
            for ie, i in enumerate(range(0, len(edge_index[0]), 1)):
                ...
                if bond:
                    node_feautures_bond_graph.append(
                        self.feature_creator.bond_features_one_hot(
                            bond, use_chirality=self.feature_creator.use_bond_chirality
                        )
                    )
                # else:
                #     node_feautures_bond_graph.append(np.zeros((12)).tolist()) # TODO: TEST THIS

            node_feautures_bond_graph = torch.tensor(
                node_feautures_bond_graph, dtype=torch.float
            )
            ...
            edge_index_bond_graph, one_bond_frag_ids = add_one_bond_frag_nodes_to_index(
                edge_index_bond_graph, bond_index_to_id, one_bond_frags
            )
            edge_attr_bond_graph = get_edge_attr_bond_graph(
                edge_index_bond_graph, idx_bond_index, conf, one_bond_frag_ids
            )

            edge_index_bond_graph = torch.tensor(
                edge_index_bond_graph, dtype=torch.int32
            )
        ...
        if self.create_fragbond_graph:
            (
                node_feautures_fragbond_graph,
                edge_index_fragbond_graph,
                edge_attr_fragbond,
            ) = get_fragbond(frag_idx, cnx_attr)
        ...
        if self.add_dhangles:
            bond_length_pair, bond_angle_true, dihedral_angle_true = (
                get_bond_angle_dhangle(conf, data.x_atoms, data.edge_index)
            )
    ...
    def get_frag_idx_cnx_attr_2s(self, graph):
        ...
        for connection in graph.connections:
            ...
            cnx_attr.append(
                self.feature_creator.connection_features_one_hot(connection)
            )
            cnx_attr.append(
                self.feature_creator.connection_features_one_hot(connection)
            )
...
def collate_fn(data_list):
    ...
    batch = torch.cat(
        [torch.zeros(data_list[i].x_atoms.shape[0]) + i for i in range(len(data_list))]
    )
    frag_batch = torch.cat(
        [torch.zeros(data_list[i].n_frags.item()) + i for i in range(len(data_list))]
    )
    ...
    # for frag graph
    node_features_fragbonds = torch.cat(
        [i.node_feautures_fbondg for i in data_list], dim=0
    )
