...
2. SMILES 解析过程中可能丢失了部分键信息

### 2.3 空字符串处理问题

在测试空字符串 SMILES 时，系统未能正确抛出 ValueError 异常。
...
1. 测试各种无效输入（不仅仅是无效 SMILES）
2. 测试极端大小的分子（非常小和非常大的分子）

### 4.2 增加化学特性专项测试

1. 专门测试各种环系统（3-8 元环，螺环，桥环等）
2. 专门测试各种杂原子（O, N, S, P 等）
